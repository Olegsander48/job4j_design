...
class AnalysisTest {
    @Test
    void whenServerWasUnavailableTwoTimes(@TempDir Path tempDir) throws IOException {
    ...
    @Test
    void whenServerWasUnavailableOneTime(@TempDir Path tempDir) throws IOException {
        File source = tempDir.resolve("server.log").toFile();
        try (PrintWriter output = new PrintWriter(source)) {
            output.println("200 10:56:01");
            output.println("500 10:57:01");
